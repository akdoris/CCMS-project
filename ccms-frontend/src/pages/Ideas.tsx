  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-end justify-between">
        <div>
          <h2 className="font-bold text-2xl text-[#0d0f14]">Content Ideas</h2>
          <p className="text-sm text-gray-400 mt-1">
            {ideas.length} ideas · {ideas.filter((i) => i.status === 'Draft').length} drafts
          </p>
        </div>
        <Button variant="gold" onClick={openNew}>+ New Idea</Button>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-gray-400">
          ...
          <p className="font-bold text-gray-700 text-base">No ideas found</p>
        ...
        <div className="grid grid-cols-3 gap-4">
          {filtered.map((idea) => (
            <div
              ...
              className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 cursor-pointer group hover:-translate-y-0.5 hover:shadow-md transition-all"
            >
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-1.5 flex-wrap">
                  <Badge variant={idea.status.toLowerCase() as 'draft'}>
                    {statusLabel(idea.status)}
                  </Badge>
                  <Badge variant={idea.platform.toLowerCase() as 'youtube'}>
                    {idea.platform}
                  </Badge>
                </div>
