...
const NOTIFICATIONS = [
  ...
  { icon: '🤝', text: 'New deal request from UrbanBrew',  time: 'Yesterday', unread: false },
]
...
export default function Topbar() {
  ...
  return (
      <header className="sticky top-0 z-40 bg-white border-b border-gray-100 h-16 px-4 md:px-8 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-1 md:gap-2">
          ...
          <div ref={searchRef} className="relative">
            <button
                onClick={() => { setShowSearch(!showSearch); setShowBell(false); setShowProfile(false) }}
                className={`p-2 rounded-xl transition-all ${showSearch ? 'bg-[#c9a84c]/10 text-[#c9a84c]' : 'text-gray-400 hover:text-gray-700 hover:bg-gray-100'}`}
            >
              🔍
            </button>
            {showSearch && (
                <div className="absolute right-0 top-12 w-72 md:w-80 bg-white rounded-2xl shadow-xl border border-gray-100 p-3 z-50">
                  <input
                      autoFocus
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#c9a84c] bg-gray-50"
                      placeholder="Search ideas, collabs, posts…"
                      value={searchVal}
                      onChange={(e) => setSearchVal(e.target.value)}
                  />
                  {searchVal && (
                      <div className="mt-2 space-y-1">
                        <p className="text-[10px] text-gray-400 uppercase tracking-wider px-2 pb-1">Quick Results</p>
                        {['Content Ideas', 'Brand Collabs', 'Schedule', 'Analytics'].map((item) => (
                            <button
                                key={item}
                                onClick={() => { setShowSearch(false); setSearchVal('') }}
                                className="w-full text-left px-3 py-2 rounded-xl text-sm hover:bg-gray-50 text-gray-700 transition-all"
                            >
                              🔍 {searchVal} in <span className="font-semibold">{item}</span>
                            </button>
                        ))}
                      </div>
                  )}
                  {!searchVal && (
                      <div className="mt-2">
                        <p className="text-[10px] text-gray-400 uppercase tracking-wider px-2 pb-2">Quick Navigate</p>
                        {[
          ...
          <div ref={bellRef} className="relative">
            <button
                onClick={() => { setShowBell(!showBell); setShowSearch(false); setShowProfile(false) }}
                className={`relative p-2 rounded-xl transition-all ${showBell ? 'bg-[#c9a84c]/10 text-[#c9a84c]' : 'text-gray-400 hover:text-gray-700 hover:bg-gray-100'}`}
            >
              🔔
              {unreadCount > 0 && (
            ...
            {showBell && (
                <div className="absolute right-0 top-12 w-80 md:w-96 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                  <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                    <div>
                      <h3 className="font-bold text-sm text-[#0d0f14]">Notifications</h3>
                  ...
                  <div className="divide-y divide-gray-50 max-h-72 overflow-y-auto">
                    {notifs.map((n, i) => (
                        <div
                            key={i}
                            className={`flex items-start gap-3 px-5 py-3.5 hover:bg-gray-50 transition-all cursor-pointer ${n.unread ? 'bg-amber-50/40' : ''}`}
                  ...
                  <div className="px-5 py-3 border-t border-gray-100">
                    <button className="text-xs text-[#c9a84c] font-semibold hover:underline w-full text-center">
          ...
          <div ref={profileRef} className="relative">
            <button
                onClick={() => { setShowProfile(!showProfile); setShowSearch(false); setShowBell(false) }}
            ...
            {showProfile && (
                <div className="absolute right-0 top-12 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                  ...
                  <div className="px-4 py-4 border-b border-gray-100">
                    <div className="flex items-center gap-3">
                      <div
                          className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-[#0d0f14]"
                          style={{ background: 'linear-gradient(135deg, #c9a84c, #2cc4a0)' }}
                      >
                        AM
                      </div>
                  ...
                  <div className="py-2">
                    {[
                      ...
                      { icon: '💬', label: 'Help & Support' },
                        ...
                        <button
                            key={item.label}
                            onClick={() => { setShowProfile(false); toast(`${item.label} — coming soon!`) }}
                            className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-all text-left"
                        >
                          <span>{item.icon}</span>
                          {item.label}
                        </button>
